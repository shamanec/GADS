...
func (d *AndroidDevice) startH264Stream() {
	...
	shellCmd := fmt.Sprintf(
		"CLASSPATH=/data/local/tmp/gads-settings app_process / com.gads.settings.server.H264Server audio_input_type=%s",
		audioInputType,
	)
	cmd := exec.CommandContext(d.Context, "adb", "-s", d.GetUDID(), "shell", shellCmd)
